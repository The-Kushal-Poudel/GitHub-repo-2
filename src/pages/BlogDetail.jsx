...
export default function BlogDetail({ data }) {
  ...
  return (
    <article className="bg-[#f5f3ee] text-[#171817]">
      ...
      <section className="bg-[#191b1a] py-12 text-white sm:py-16 lg:py-20">
        <Container className="max-w-[1040px]">
          ...
          <header className="mt-12">
            ...
            <h1 className="text-balance mt-5 text-4xl font-black leading-[0.97] tracking-[-0.058em] sm:text-6xl lg:text-7xl">{blog.title}</h1>
      ...
      <Container className="max-w-[980px] py-12 sm:py-16">
        <div className="mx-auto max-w-[760px]">
          {blog.content.map((paragraph, index) => (
            <p key={paragraph} className={`mb-7 text-[17px] leading-8 text-black/64 ${index === 0 ? "first-letter:float-left first-letter:mr-3 first-letter:text-6xl first-letter:font-black first-letter:leading-[.82] first-letter:text-[#3557c8]" : ""}`}>{paragraph}</p>
          ))}
        </div>
      </Container>
    </article>
